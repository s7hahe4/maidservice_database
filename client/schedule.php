...
<body>
    ...
    <div class="container">
        ...
        if ($_POST) {
            $search = $_POST['search'];
            $sqlmain = "SELECT maid.maidid, maid.maidname, maid.maidemail, specialties.sname, schedule.scheduleid, schedule.title, schedule.scheduledate, schedule.scheduletime, schedule.`fees/hr`, schedule.`cost/delay` 
                        FROM schedule 
                        INNER JOIN maid ON schedule.maidid = maid.maidid 
                        LEFT JOIN specialties ON maid.specialties = specialties.id
                        WHERE (maid.maidname LIKE '%$search%' 
                        OR maid.maidemail LIKE '%$search%' 
                        OR specialties.sname LIKE '%$search%')
                                            ...
                                            echo '<td style="width: 25%;">
                                                <div class="dashboard-items search-items">
                                                    <div>
                                                        ...
                                                        <div class="h4-search">Date: ' . htmlspecialchars($row['scheduledate']) . '<br>Time: <b>@' . htmlspecialchars($startTime->format('H:i:s')) . ' to ' . htmlspecialchars($endTime->format('H:i:s')) . '</b></div>
                                                        <div class="h4-search">Fees/Hour: ৳' . htmlspecialchars($row['fees/hr']) . '</div>
                                                        <div class="h4-search">Cost/Delay: ৳' . htmlspecialchars($row['cost/delay']) . '</div>
