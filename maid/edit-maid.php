...
if($_POST){
    ...
    if ($password==$cpassword){
        $error='3';
        $result= $database->query("select maid.maidid from maid inner join webuser on maid.maidemail=webuser.email where webuser.email='$email';");
        ...
        if($id2!=$id){
            ...
            $sql1="update maid 
                   set maidemail='$email',maidname='$name',maidpassword='$password',maidnic='$nic',maidtel='$tele',specialties=$spec 
                   where maidid=$id ;";
            $database->query($sql1);
